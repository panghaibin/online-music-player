    ...
    $(".music-list").on("mousemove",".list-item", function() {
        ...
        if(!$(this).data("loadmenu")) {
            ...
            '<div class="list-menu" data-no="' + num + '">' +
                ...
                '<span class="list-icon icon-share" data-function="share" title="点击获取PDF歌词"></span>' +
            '</div>';
            target.html(html);
...
function musicInfo(list, index) {
    ...
    var tempStr = '<span class="info-title">阻止屏幕休眠：</span> <input type="button" id="nosleep" value="点击开启" />' +
    '<br><span class="info-title">歌名：</span>' + music.name + 
    ...
    '<span style="margin-left: 10px" class="info-btn" onclick="thisShare(this)" data-list="' + list + '" data-index="' + index + '">获取PDF歌词</span>' + 
    '<script>'+
...
*/
    ...
    '<input id="filename" class="share-url" onmouseover="this.focus();this.select()" value="' + music.artist + ' - ' + music.name + '">' + 
    '<button class="layui-layer-btn0" data-clipboard-action="copy" data-clipboard-target="#filename" style="height: 35px;">复制</button>' +
    '<p>歌曲链接：</p>' + 
    '<input id="url" class="share-url" onmouseover="this.focus();this.select()" value="' + music.url + '">' + 
    //'<button class="layui-layer-btn0" data-clipboard-action="copy" data-clipboard-target="#url" style="height: 35px;">复制</button>' +
    //'<button class="layui-layer-btn0 music-url" onclick="window.open(\'' + music.url + '\')" style="height: 35px;">下载</button>' +
    '<a href="' + music.url +'" target="_blank"><button class="layui-layer-btn0 music-url"style="height: 35px;">下载</button></a>' +
    '<p>LRC 歌词下载：<a class="info-btn" target="_blank" href="/lyricpress/lrc-down.php?types=lyric&id=' + music.lyric_id + "&source=" + music.source + "&name=" + music.name + "&artist=" + music.artist + '">点击下载</a></p>' + 
    '<p>专辑封面链接：</p>' + 
    '<input id="pic" class="share-url" onmouseover="this.focus();this.select()" value="' + music.pic + '">' + 
    //'<button class="layui-layer-btn0" data-clipboard-action="copy" data-clipboard-target="#pic" style="height: 35px;">复制</button>' +
    '<button class="layui-layer-btn0 pic-url" onclick="window.open(\'' + music.pic + '\')" style="height: 35px;">下载</button>' +
    '<p class="share-tips">* 若封面链接显示 null ，请先点击播放按钮再下载<br>* 复制文件名后再下载，方便重命名</p>';
...
function ajaxShare(music) {
    ...
    var tmpHtml = 
    '<form target="_blank" method="post" action="/lyricpress/pdf-down.php">' +
    '<input type="hidden" name="types" value="lyric">' +
    '<input type="hidden" name="id" value="' + music.lyric_id + '">' +
    '<input type="hidden" name="source" value="' + music.source + '">' +
    '文件名：<input type="text" name="name" value="' + music.artist + ' - ' + music.name + '">.pdf <br>' +
    '字&nbsp;&nbsp;&nbsp;体：' +
    '<select name="font-family">' +
    '<option value="00" selected>等线</option>' +
    '<option value="01">微软雅黑</option>' +
    '<option value="02">华康少女体</option>' +
    '</select>' +
    '<br>字&nbsp;&nbsp;&nbsp;号：' +
    ...
    '<option value="2" selected>两栏</option>' +
    '<option value="3">三栏</option>' +
    '</select>' +
    '<br>二维码：<input type="radio" name="qr-code" value="on" checked>开启 <input type="radio" name="qr-code" value="off">关闭' +
    '<br><input type="submit" value="生成">' +
    '</form>' +
    '';
    
    layer.open({
        title: '将歌词制作成PDF(Beta)'
        ,content: tmpHtml
    });
}
